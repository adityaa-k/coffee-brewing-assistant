...
function BrewerApp() {
  ...
  // This component will now manage its own state for the Iced V60 guide
  const IcedV60Guide = () => {
    ...
    const [settings, setSettings] = useState(() => getInitialState('cba_settings', { gramsPerPerson: 18, coffeeRatio: 65, waterSplit: 60, bloomRatioMin: 2, bloomRatioMax: 3, bloomTimeMin: 45, bloomTimeMax: 55 }));
    const [useCustomBloom, setUseCustomBloom] = useState(() => getInitialState('cba_useCustomBloom', false));
    const [customBloomConfig, setCustomBloomConfig] = useState(() => getInitialState('cba_customBloomConfig', { count: 2, durations: [45, 45] }));

    useEffect(() => { try { if (typeof window !== 'undefined') { window.localStorage.setItem('cba_settings', JSON.stringify(settings)); } } catch (e) { console.error(e) } }, [settings]);
    useEffect(() => { try { if (typeof window !== 'undefined') { window.localStorage.setItem('cba_useCustomBloom', JSON.stringify(useCustomBloom)); } } catch (e) { console.error(e) } }, [useCustomBloom]);
    useEffect(() => { try { if (typeof window !== 'undefined') { window.localStorage.setItem('cba_customBloomConfig', JSON.stringify(customBloomConfig)); } } catch (e) { console.error(e) } }, [customBloomConfig]);
    useEffect(() => { try { if (typeof window !== 'undefined') { window.localStorage.setItem('cba_personalBrews', JSON.stringify(personalBrews)); } } catch (e) { console.error(e) } }, [personalBrews]);
    ...
    const handleStartBrewing = () => { 
        trackEvent('start_brew', { brew_method: 'iced-v60', coffee_weight: brewConfig.coffeeWeight, is_custom_recipe: useCustomBloom, bloom_count: brewConfig.bloomCount });
        setCurrentStep('brewing'); setCurrentBloom(0); setTimeLeft(brewConfig.bloomTimes[0]); 
    };
    ...
    const handleCompleteBrew = () => { 
        trackEvent('complete_brew', { brew_method: 'iced-v60', coffee_weight: brewConfig.coffeeWeight, total_brew_time: brewConfig.bloomTimes.reduce((a, b) => a + b, 0) });
        setPersonalBrews(prev => prev + 1); setCurrentStep('complete'); 
    };
    const handleSettingChange = (settingName, value) => {
        trackEvent('customize_recipe', { setting_changed: `iced_v60_${settingName}`, new_value: value });
        setSettings(s => ({...s, [settingName]: value}));
    };
    const handleCustomBloomToggle = () => {
        trackEvent('customize_recipe', { setting_changed: 'iced_v60_use_custom_bloom', new_value: !useCustomBloom });
    ...
    const renderBrewAppContent = () => {
        switch(currentStep) {
            ...
            case 'setup': default: return renderSetupPhase();
        }
    };
        ...
        <div className="space-y-6 text-brand-brown">
            ...
            <Card>
                ...
                <ul className="list-disc list-inside mt-2 text-brand-brown/80 space-y-1">
                    <li><span className="font-semibold text-brand-brown">v2.7.0 (Cold Brew):</span> Added Cold Brew guide and fixed navigation.</li>
    ...
    let contentToRender;
    switch (currentPage) {
        case 'settings': contentToRender = renderSettings(); break;
        case 'info': contentToRender = renderInfoPage(); break;
        case 'app': default: contentToRender = renderBrewAppContent(); break;
    }
    ...
    return (
        <div className="pb-24">
            ...
            <div className="fixed bottom-0 left-0 right-0 bg-white/60 backdrop-blur-md border-t border-black/10 shadow-t-sm max-w-lg mx-auto">
                <nav className="flex justify-around items-center h-20 px-4">
                    <BottomNavItem icon={Coffee} label="Brew" pageName="app" />
                    <BottomNavItem icon={SlidersHorizontal} label="Settings" pageName="settings" />
                    <BottomNavItem icon={Info} label="Info" pageName="info" />
                </nav>
            </div>
        </div>
    );
  ...
  return (
    <>
      <Seo
        title="Brewer App | Coffee Brewing Assistant"
        description="Use our interactive brewing assistant for Iced V60 and Cold Brew. Get adaptive timers and recipes for the perfect cup."
        canonicalUrl="https://coffeebrewingassistant.com/app"
        imageUrl="https://coffeebrewingassistant.com/og-image-app.png"
      />
      <div className="container mx-auto max-w-lg p-4">
          ...
          <main>
              {activeBrewMethod === 'iced-v60' && <IcedV60Guide />}
